    ...
    <div id="tagsBlogs">
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <span style="font-size: 0.5rem; color: #00B5AD;">标签</span>
...
<script>
    ...
    export default {
      ...
      methods:{
        selectAllTag(page , rows){
          page = page ? page : this.page;
          rows =rows ? rows : this.rows;
          this.$http.get("http://39.106.86.151:8989/blog/selectAllTag?page=" + page +"&rows=" + rows + "&id=" + this.$route.params.id).then(res=>{
        ...
        findSize(size) { //用来处理每页显示记录发生变化的方法
          ...
          if(this.$route.params.id != undefined){
            this.selectAllTag(this.page,size);
          }else{
        ...
        findPage(page){
          this.page = page
          if(this.$route.params.id != undefined){
            this.selectAllTag(page,this.rows);
          }else{
